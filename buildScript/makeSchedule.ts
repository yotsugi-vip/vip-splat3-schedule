import { createCanvas, registerFont, Image, loadImage, Canvas } from 'canvas';
...
const makeAllImg = async (schedule_json: splat3api) => {
...
const scheduleTimeImg = async (schedule_json: splat3api) => {
...
const StageImg = async (schedule_json: splat3api, rule: string) => {
    registerFont('./public/Koruri-20210720/Koruri-Regular.ttf', { family: 'kokuri_r' });
    registerFont('./public/Koruri-20210720/Koruri-Extrabold.ttf', { family: 'kokuri_exb' });
    ...
    for (let i: number = 0; i < drawRule.length; i++) {
        ...
        if (!drawRule[i].is_fest) {
            for (const stage of drawRule[i].stages) {
                ...
                if (rule === "open") stg_ctx.fillText("オープン", 10, 60);
                if (rule === "challenge") stg_ctx.fillText("チャレンジ", 10, 60);
                if (rule === "x") {
            ...
            if (rule === "open") {
                ...
                const stage_cvs = createCanvas(w, h);
                const stg_ctx = stage_cvs.getContext('2d');
                const get_stageID = ((name: string): number => {
                    let ret: number = 0;
                    switch (name) {
                        case "ユノハナ大渓谷":
                            ret = 1;
...
const getStageImage = async (stage_id: number) => {
    let imgbuff = new Image();
    try {
...
const getRuleAsset = (rule: string) => {
...
const clipCarveSq = (image: Canvas) => {
